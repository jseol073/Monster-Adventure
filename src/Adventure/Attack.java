...
public class Attack {
    ...
    /**
     * Used when user attacks without an item
     * Ends game if player dies
     * increases player's experience when monster dies
     * @param maybeItemName
     * @param roomIndex
     * @param layout
     * @return
     */
    public static String attack(String maybeItemName, int roomIndex, Layout layout) {
        Monster currMonster = Dual.getMonsterInRoom(maybeItemName, roomIndex, layout);
    ...
    /**
     * Used when user attacks with an item
     * Ends game if player dies
     * increases player's experience when monster dies
     * @param maybeItemName
     * @param roomIndex
    ...
    public static String increaseExp(int plyrLevel, Monster currMonster) {
        double experience = (((currMonster.getAttack() + currMonster.getDefense()) / 2)
        ...
        if (plyrLevel <= 2) {
            if (plyrLevel == 1) {
                if (experience >= EXP_LEVEL_1) {
                    ...
                    Adventure.player.setFullHealth(Adventure.player.getFullHealth() * Math.pow(1.3, plyrLevel));
